            ...
            steps {
                script {
                    ...
                    withCredentials([string(credentialsId: env.GITHUB_TOKEN, variable: 'GITHUB_TOKEN')]) {
                        existingRelease = sh(script: "curl -sSL -H 'Accept: application/vnd.github+json' -H 'Authorization: Bearer $GITHUB_TOKEN' -H 'X-GitHub-Api-Version: 2022-11-28' https://api.github.com/repos/${GITHUB_REPO_FULL}/releases", returnStdout: true).trim()
                    }
                    ...
                    withCredentials([string(credentialsId: env.GITHUB_TOKEN, variable: 'PAT')]) {
                        dir(path: env.BUILD_ID) {
